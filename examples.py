...
from utils import model_translations
from model import make_model
from data.dataloaders import load_tokenizers, load_vocab
...
def main(cfg: DictConfig):
    ...
    example_data = check_outputs(test_dataloader, model, vocab_src, vocab_tgt, n_examples=cfg.n_examples)
    return
...
def check_outputs(
    ...
    for idx in range(n_examples):
        ...
        sentences_txt = (
            " ".join(model_txt).split(eos_string, 1)[0]
            + eos_string
        )
...
def greedy_decode(model, src, src_mask, max_len, start_symbol):
    ...
    for i in range(max_len - 1):
        out = model.decode(
            memory, src_mask, sentence, subsequent_mask(sentence.size(1)).type_as(src.data)
        )
        prob = model.generator(out[:, -1])
        _, next_word = prob.max(dim=1)
        next_word = next_word.data[0]
        sentence = torch.cat(
            [sentence, torch.Tensor([[next_word]]).type_as(src.data)], dim=1
        )
    return sentence
